...
typedef enum MK2Status
{
  ...
  MK2STATUS_FAILURE_RX_FCS_ERROR,
  MK2STATUS_FAILURE_RX_LENGTH_MISMATCH,
} eMK2Status;
/// @copydoc eMK2Status
...
#endif
/// Request the minimum transmit power supported by the PHY
#define MK2POWER_MIN             (INT16_MIN)

/// Request the maximum transmit power supported by the PHY
#define MK2POWER_MAX             (INT16_MAX)
...
typedef struct MK2TxDescriptor
{
  ...
  /// Indicate the expiry time as an absolute MAC time in microseconds (0 means never)
  tMK2Time Expiry;
...
typedef struct MK2RxMeta
{
  ...
  /// Current load on the channel (ratio of channel busy to idle time),
  /// measured over the last measurement period. 255 = 100%
  uint8_t ChannelUtilisation;
  /// Current channel utilisation measurement period in TU units
  /// (1 TU = 1.024 ms)
  uint16_t ChannelUtilisationPeriod;
...
/// Convert ms to nearest TU (1 TU = 1.024 ms)
#define MK2MAC_MS_TO_TU(ms) ((((ms) * 1000) + 1023) / 1024)
